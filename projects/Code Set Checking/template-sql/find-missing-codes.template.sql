...
--> CODESET tuberculosis:1 hepatitis-a:1 hepatitis-b:1 hepatitis-c:1 hepatitis-d:1
...
IF OBJECT_ID('tempdb..#TEMP_Concepts') IS NOT NULL DROP TABLE #TEMP_Concepts;
select distinct Concept INTO #TEMP_Concepts from #AllCodes;


DECLARE @Iteration INT; 
DECLARE @NewInsertions INT; 
DECLARE @Concept VARCHAR(255);
WHILE EXISTS (SELECT * FROM #TEMP_Concepts)
BEGIN
	SELECT TOP 1 @Concept = Concept FROM #TEMP_Concepts ORDER BY Concept ASC
	...
	BEGIN  
		...
		-- First we find all foreign keys based on existing codes
			...
			INTO #FKsFromReadv2
			FROM SharedCare.Reference_Coding
			WHERE EXISTS (SELECT * FROM #codesreadv2 WHERE Code = MainCode AND (Term = SharedCare.Reference_Coding.Term OR Term IS NULL) and concept = @Concept)
			...
			INTO #FKsFromCTV3
			FROM SharedCare.Reference_Coding
			WHERE EXISTS (SELECT * FROM #codesctv3 WHERE Code = MainCode AND (Term = SharedCare.Reference_Coding.Term OR Term IS NULL) and concept = @Concept)
			AND CodingType='CTV3'
			AND PK_Reference_Coding_ID!=-1;
