...
-- 1. Create a folder under the "Worksheets" tab within Snowflake named SDE-Lighthouse-01-Newman (unless it already exists)
-- 2. For each sql file in the extraction-sql directory, create a worksheet with the same name within the SDE-Lighthouse-01-Newman folder
...
--    e. Select the table: "Code_Sets_SDE_Lighthouse_01_Newman" and click "Next"
--    f. Select the file format "Delimited files (CSV/TSV)"
--    g. Double check that the preview looks ok and then click "Load"
-- 6. You can now return to the worksheet folder SDE-Lighthouse-01-Newman and execute the remaining sql files.

USE SDE_REPOSITORY.SHARED_UTILITIES;

-- Creates the code set table for this project.
DROP TABLE IF EXISTS "Code_Sets_SDE_Lighthouse_01_Newman";
CREATE TABLE "Code_Sets_SDE_Lighthouse_01_Newman" (
	CONCEPT VARCHAR(255),
	VERSION NUMBER(38,0),
...
CREATE TABLE IF NOT EXISTS "Patient_ID_Mapping_SDE_Lighthouse_01_Newman" (
...
DROP FUNCTION IF EXISTS gm_pseudo_hash_SDE_Lighthouse_01_Newman(NUMBER(38,0));
CREATE FUNCTION gm_pseudo_hash_SDE_Lighthouse_01_Newman("GmPseudo" NUMBER(38,0))
  RETURNS NUMBER(38,0)
  AS
  $$
    SELECT MAX("StudyPatientPseudoId")
    FROM SDE_REPOSITORY.SHARED_UTILITIES."Patient_ID_Mapping_SDE_Lighthouse_01_Newman"
