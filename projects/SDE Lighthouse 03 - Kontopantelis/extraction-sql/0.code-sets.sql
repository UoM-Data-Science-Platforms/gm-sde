...
-- 1. Create a folder under the "Worksheets" tab within Snowflake named SDE-Lighthouse-03-Kontopantelis (unless it already exists)
-- 2. For each sql file in the extraction-sql directory, create a worksheet with the same name within the SDE-Lighthouse-03-Kontopantelis folder
...
-- 6. You can now return to the worksheet folder SDE-Lighthouse-03-Kontopantelis and execute the remaining sql files.

USE SDE_REPOSITORY.SHARED_UTILITIES;

-- Creates the code set table for this project.
DROP TABLE IF EXISTS "Code_Sets_SDE_Lighthouse_03_Kontopantelis";
CREATE TABLE "Code_Sets_SDE_Lighthouse_03_Kontopantelis" (
	CONCEPT VARCHAR(255),
	VERSION NUMBER(38,0),
...
CREATE TABLE IF NOT EXISTS "Patient_ID_Mapping_SDE_Lighthouse_03_Kontopantelis" (
...
DROP FUNCTION IF EXISTS gm_pseudo_hash_SDE_Lighthouse_03_Kontopantelis(NUMBER(38,0));
CREATE FUNCTION gm_pseudo_hash_SDE_Lighthouse_03_Kontopantelis("GmPseudo" NUMBER(38,0))
  RETURNS NUMBER(38,0)
  AS
  $$
    SELECT MAX("StudyPatientPseudoId")
    FROM SDE_REPOSITORY.SHARED_UTILITIES."Patient_ID_Mapping_SDE_Lighthouse_03_Kontopantelis"
