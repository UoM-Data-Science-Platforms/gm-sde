...
WHERE SCTID IN (SELECT "SCTID" FROM TEMP_LH004_VAC_CODES)
AND "FK_Patient_ID" IN (SELECT "FK_Patient_ID" FROM SDE_REPOSITORY.SHARED_UTILITIES."Cohort_SDE_Lighthouse_04_Bruce");
...
WHERE SCTID IN (
	SELECT "SCTID" FROM TEMP_LH004_VAC_CODES
...
SELECT DISTINCT "GmPseudo" FROM SDE_REPOSITORY.SHARED_UTILITIES."LH004-5_vaccinations_WITH_IDENTIFIER"
EXCEPT
...
SELECT SDE_REPOSITORY.SHARED_UTILITIES.gm_pseudo_hash_SDE_Lighthouse_04_Bruce("GmPseudo") AS "PatientID",
	* EXCLUDE "GmPseudo"
FROM SDE_REPOSITORY.SHARED_UTILITIES."LH004-5_vaccinations_WITH_IDENTIFIER";