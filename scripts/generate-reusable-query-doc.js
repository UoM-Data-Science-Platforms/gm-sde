...
const REUSABLE_QUERY_DIRECTORY = join(__dirname, '..', 'shared', 'Reusable queries for data extraction')
...
--└───────────────────────────┘
  ...
  if(!properties.OBJECTIVE) {
    ...
    console.warn(`${query} does not have an INPUT. This is not required but is advised. If the script has no input, please add the following:`);
    console.log('-- INPUT: No pre-requisites');
    objectives.push({name: properties.NAME, query, text: properties.OBJECTIVE, output: properties.OUTPUT})
  } else{
    objectives.push({
...
function parseQuery(queryFilename) {
  ...
  while(trimmedLine === '' || trimmedLine.indexOf('--') === 0) {
    if(trimmedLine.indexOf('--│') === 0 || trimmedLine.indexOf('--|') === 0) {
      ...
      let [,field, content] = trimmedLine.match(/^-- ([A-Z]+):(.*)$/) || [,,trimmedLine];
      if(field) {
        currentField = field;
        if(fieldsAsString.indexOf(field) > -1) {
          ...
          if(content.trim()==='') {
            ...
            console.log(`In file ${queryFilename} the field ${field} should be a list but it isn't. The format is:`);
          ...
          console.log(`The file ${queryFilename} has a field in the comments (${field}) that is not recognised.`)
        }        
        ...
        if(fieldsAsString.indexOf(currentField) > -1) {
          ...
          let [isNew,text] = content.substr(2).trim().match(/^-\s*(.+)$/) || [false, content.substr(2).trim()];
          if(isNew){
            // new item
...
`).join('\n---\n')}`;
  ...
  writeFileSync(join(REUSABLE_QUERY_DIRECTORY, 'README.md'), readme);
}
